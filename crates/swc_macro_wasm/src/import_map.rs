...
impl ImportMap {
    ...
    ///
    /// ```
    pub fn get_require_path(&self, e: &Expr) -> Option<Atom> {
        match e {
            Expr::Call(CallExpr {
                ...
                if callee.sym == "require" {
                    return if let Some(Expr::Lit(Lit::Str(s))) = args.first().map(|a| &*a.expr) {
                        Some(s.value.clone())
                    } else {
                        None
                    };
                ...
                if self.require_wrappers.contains(&callee.to_id()) {
                    if let Some(Expr::Lit(Lit::Str(s))) = args.first().map(|a| &*a.expr) {
                        Some(s.value.clone())
                    } else {
                        None
                    }
